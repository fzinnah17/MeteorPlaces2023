...
//             <span ref={rotationRef} className="rotate" />
...
const Landing = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (rotationElement) {
        rotationElement.classList.add("fade-out");

        setTimeout(() => {
          rotationElement.textContent = words[currentIndex];
          rotationElement.classList.remove("fade-out");
        }, 500);
      }
    }, 2000);
